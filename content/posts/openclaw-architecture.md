...
│  ┌─────────────┐  ┌────────────┐  ┌───────────┐  ┌──────────────────┐  │
│  │    渠道     │  │   Agent    │  │   会话    │  │    工具执行器    │  │
│  │  (Baileys,  │  │   路由器   │  │   管理器  │  │  (主机/沙箱)     │  │
│  │   grammY)   │  │            │  │           │  │                  │  │
│  └─────────────┘  └────────────┘  └───────────┘  └──────────────────┘  │
│                                                                          │
│  ┌─────────────┐  ┌────────────┐  ┌───────────┐  ┌──────────────────┐  │
│  │    配置     │  │    定时    │  │  Webhook  │  │   Canvas 主机    │  │
│  │    热重载   │  │   调度器   │  │   处理器  │  │  (端口 18793)    │  │
│  │            │  │            │  │           │  │                  │  │
│  └─────────────┘  └────────────┘  └───────────┘  └──────────────────┘  │
└─────────────────────────────┬───────────────────────────────────────────┘
                              │
...
┌───────────────┐    ┌────────────────┐    ┌───────────────────────┐
│     CLI       │    │   macOS 应用   │    │  设备节点             │
│ (openclaw ...) │    │  (菜单栏)      │    │ (iOS/Android/macOS)   │
└───────────────┘    └────────────────┘    └───────────────────────┘
```
...
│  2. 验证参数，解析会话                                       │
│         ↓                                                   │
│  3. 返回 {runId, acceptedAt}                               │
│         ↓                                                   │
│  4. agentCommand 执行 agent：                               │
│     - 加载技能快照                                          │
│     - 构建系统提示                                          │
│     - 调用 runEmbeddedPiAgent                              │
│         ↓                                                   │
│  5. 模型推理 + 工具调用                                     │
│     - 发射流式事件（tool, assistant）                       │
│     - 执行工具（主机或沙箱）                                 │
│         ↓                                                   │
│  6. 发射生命周期结束/错误事件                                │
│         ↓                                                   │
